    ...
    actions: {
        ...
        handleSubmitIndividual(clientObject: Object) {
            ...
            fetch("/api/save-individual", {
                ...
                .then((response) => {
                    if (response.ok) {
                        this.success.success_individual =
                            "Client saved successfully";
                    } else {
                        this.error.error_individual =
        ...
        handleSubmitAll(clientsArray: Array<ClientData>) {
            ...
            fetch("/api/save-clients", {
                ...
                .then((response) => {
                    if (response.ok) {
                        this.success.success_all =
                            "All clients saved successfully";
                    } else {
                        this.error.error_all =
                            "Failed to save client data, please try again";
        ...
        async uploadCSV(file: File | null) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    this.updateClientData(data.data);
                } else {
                    this.loading = false;
                    this.error.error_upload =
